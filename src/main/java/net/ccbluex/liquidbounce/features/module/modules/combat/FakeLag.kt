            ...
            if (packet.javaClass.name.startsWith("net.minecraft.network.play.server.")) {
                ...
                when (packet) {
                    is S0CPacketSpawnPlayer -> {
                        if (entityPosMap.containsKey(packet.entityID)) return
                        val pd = PosData()
                        pd.prevX = (packet.x.also { pd.x = it }) / 32.0
                        pd.prevY = (packet.y.also { pd.y = it }) / 32.0
                        pd.prevZ = (packet.z.also { pd.z = it }) / 32.0
